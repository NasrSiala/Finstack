...
def main(args=None):
    ...
    if HAS_KAFKA and os.environ.get('FINCEPT_KAFKA_BROKER'):
        try:
            producer = FinceptKafkaProducer()
            topic = "market-data-minute"
            
            # If it's a list (batch), send each item
            if isinstance(result, list):
                producer.produce(topic, result)
            # If it's a single quote (dict)
            elif isinstance(result, dict) and "error" not in result:
                producer.produce(topic, result)
                
            producer.flush()
        except Exception:
            pass # Never break the main terminal flow
